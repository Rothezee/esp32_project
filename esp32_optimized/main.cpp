/*
ESP32 Grúa Controller - Versión Optimizada 2025
Características:
...
- Compatibilidad con sistema web moderno
*/
...
struct Config {
    ...
    // URLs del servidor
    static constexpr const char* SERVER_DATA_URL = "https://maquinasbonus.com/api/esp32/data";
    static constexpr const char* SERVER_HEARTBEAT_URL = "https://maquinasbonus.com/api/esp32/heartbeat";
...
public:
    bool initialize() {
        if (!preferences.begin("storage", false)) {
            Serial.println("Error inicializando Preferences");
            return false;
        }
        
        // Verificar si es la primera vez
        int inicio = preferences.getInt("inicio", 0);
        if (inicio != 35) {
            resetToDefaults();
    ...
    void loadFromMemory() {
        counters.coin = preferences.getInt("coin", 0);
        counters.contsalida = preferences.getInt("contsalida", 0);
        counters.bank = preferences.getInt("bank", 0);
        counters.pago = preferences.getInt("pago", 12);
        counters.pjfijo = preferences.getInt("pjfijo", 0);
        counters.ppfijo = preferences.getInt("ppfijo", 0);
        
        settings.tiempo = preferences.getInt("tiempo", 2000);
        settings.fuerza = preferences.getInt("fuerza", 50);
        settings.tiempo5 = preferences.getInt("tiempo5", 0);
        settings.barreraaux2 = preferences.getInt("barreraaux2", 0);
        settings.gruadisplay = preferences.getInt("gruaDisplay", 0);
    }
    
    void saveCounters() {
        preferences.putInt("coin", counters.coin);
        preferences.putInt("contsalida", counters.contsalida);
        preferences.putInt("bank", counters.bank);
        preferences.putInt("pjfijo", counters.pjfijo);
        preferences.putInt("ppfijo", counters.ppfijo);
    }
    
    void saveSettings() {
        preferences.putInt("pago", counters.pago);
        preferences.putInt("tiempo", settings.tiempo);
        preferences.putInt("fuerza", settings.fuerza);
        preferences.putInt("tiempo5", settings.tiempo5);
        preferences.putInt("barreraaux2", settings.barreraaux2);
        preferences.putInt("gruaDisplay", settings.gruadisplay);
    }
};
...
void setup() {
    Serial.begin(115200);
    Serial.println("=== ESP32 Grúa Controller v2.0 ===");
    ...
    analogWriteFrequency(Config::PIN_EPINZA, 100);
...
void showInitialInfo() {
    displayManager.showMessage("VERSION 2.0", "ESP32 Optimized");
    delay(2000);
...
void enterProgrammingMode() {
    ...
    // Mostrar versión
    displayManager.showMessage("VERSION 2.0", "Optimized");
    delay(500);
    while (digitalRead(Config::PIN_DATO3) == HIGH) { delay(20); }
...
void programResetCounters() {
    ...
    if (resetCounters) {
        machineData.resetToDefaults();
        machineData.loadFromMemory();
...
void testBarrier() {
    ...
    while (digitalRead(Config::PIN_DATO3) == HIGH) {
        ...
        displayManager.showMessage("PRUEBA BARRERA", String(distance));
