...
function calcularCierresDiarios(reports) {
    ...
    reports.forEach(report => {
        ...
        if (!cierresPorDia[fecha]) {
            cierresPorDia[fecha] = { fecha, pesos: 0, coin: 0, premios: 0, banco: 0 };
        }

        cierresPorDia[fecha].pesos += report.dato1;
        cierresPorDia[fecha].coin += report.dato2;
        cierresPorDia[fecha].premios += report.dato3;
        cierresPorDia[fecha].banco += report.dato4;
    });
...
function calcularCierreMensual(fechaInicio) {
    ...
    const totalMes = diasDelMes.reduce((acc, dia) => {
        ...
        acc.banco += dia.banco;
        return acc;
    }, { fecha: `${inicioMes.toLocaleDateString()} - ${finMes.toLocaleDateString()}`, pesos: 0, coin: 0, premios: 0, banco: 0 });
